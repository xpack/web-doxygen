...
<a id="default_file_extension_mapping"></a>Doxygen looks at the file's extension to determine how to parse a file, using the following table:
...
Please note that the above list might contain more items than that by default set in the <a href="/web-doxygen/docs/pages/config/#cfg_file_patterns">FILE\_PATTERNS</a>.
...
Doxygen can also generate output in the <a href="https://docbook.org/">DocBook</a> format. How to process the DocBook output is beyond the scope of this manual.
...
The text inside a special documentation block is parsed before it is written to the HTML and/or <code>{\LaTeX}</code> output files.
